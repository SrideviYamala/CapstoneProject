...
public class BankAccountRepository implements BankAccountDAO{
	 private final Connection conn; 
		...
		public List<BankAccount> showAllBankAccountsById(int accNo) throws SQLException{
			 PreparedStatement ps = conn.prepareStatement("""
                       select * from bankaccounts where AccNo=?
                     """);
            ps.setInt(1,accNo);
			  List<BankAccount> bankAccList = new ArrayList<>();
				 ResultSet rs = ps.executeQuery();
		...
		public BankAccount showBalance(Integer accNo)
	            throws SQLException{
	       
	        PreparedStatement ps = conn.prepareStatement("select accBalance from bankaccounts where accNo=?");
	        ps.setInt(1,accNo);
	        ResultSet rs = ps.executeQuery();
	        rs.next();
	        return new BankAccount(rs.getDouble(1));
